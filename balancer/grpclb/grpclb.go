...
// enterFallbackLocked is called when an explicit fallback signal is received
// from the remote balancer. It switches to using resolved backend addresses.
func (lb *lbBalancer) enterFallbackLocked() {
	lb.mu.Lock()
	defer lb.mu.Unlock()
	lb.refreshSubConns(lb.resolvedBackendAddrs, true, lb.usePickFirst)
}
